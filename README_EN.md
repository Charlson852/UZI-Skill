...
**A-share / HK / US deep-analysis engine — with first-class Chinese-market coverage Western terminals don't touch. v3.3.1: Hermes compatibility regression fix (main now ships skill-dir entrypoint + requirements.txt) · v3.2.0: architectural split of `assemble_report.py` (-80%) · v3.1.0: `run_real_test.py` refactor (-65%) · v3.0.0: pipeline architecture default. v2.x APIs 100% compatible via re-export · 332 pytest pass.**
